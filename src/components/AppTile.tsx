    ...
    >
      ...
      <div className={cn("relative h-32 w-full overflow-hidden", !allowed && "opacity-90")}>
        {app.coverUrl ? (
          ...
          >
            {/* soft highlight + oversized monogram for a techy, branded cover */}
            <span
        ...
        <ProgressiveBlur
          className="pointer-events-none absolute bottom-0 left-0 h-3/4 w-full"
          blurIntensity={6}
        />
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {app.maintainer?.name && (
          <div className="mt-3 flex items-center gap-2">
            <Avatar
              name={app.maintainer.name}
              src={app.maintainer.avatarUrl}
              size={22}
              color={app.color}
            />
            <span className="min-w-0 truncate text-[11.5px] text-ink-mute">
              <span className="text-ink-mute">{t.apps.maintainedBy} </span>
              <span className="font-semibold text-ink-soft">{app.maintainer.name}</span>
            </span>
          </div>
        )}
